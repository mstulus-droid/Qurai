...
# Cari index kolom CONTRADICTIONS
col_idx = headers.index('CONTRADICTIONS') + 1  # 1-based

# Fungsi untuk menambahkan kontradiksi
def add_contradiction(surat, ayat, text):
    for row_idx in range(2, ws.max_row + 1):
        cell_surat = ws.cell(row=row_idx, column=headers.index('surat') + 1).value
        cell_ayat = ws.cell(row=row_idx, column=headers.index('ayat') + 1).value
        if cell_surat == surat and cell_ayat == ayat:
            current = ws.cell(row=row_idx, column=col_idx).value
            if current is None or str(current).strip() == '':
                ws.cell(row=row_idx, column=col_idx, value=text)
                print(f'Added contradiction for Surat {surat} Ayat {ayat}')
            else:
                print(f'Skipped Surat {surat} Ayat {ayat} (already has contradiction)')
            return
    print(f'NOT FOUND: Surat {surat} Ayat {ayat}')
