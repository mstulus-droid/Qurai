...
export function SurahStatsPanel({ verses, surahNameLatin }: SurahStatsPanelProps) {
  ...
  const popupVerses = useMemo<PopupVerse[]>(() => {
    if (!activePopup) {
      return [];
    }
  ...
  return (
    <>
      <div className="qurai-card rounded-[1.5rem] px-4 py-5 sm:px-6">
          ...
          {stats.map((stat) => {
            ...
            return (
              <button
                ...
                className={`relative overflow-hidden rounded-2xl border border-[var(--qurai-border)] bg-[color-mix(in_srgb,var(--qurai-surface-strong)_78%,transparent)] px-3 py-4 text-center transition hover:border-[var(--qurai-border-strong)] sm:px-4`}
              >
                <p className={`text-2xl font-bold ${stat.color} sm:text-3xl`}>{percentage}%</p>
                <p className="mt-1 text-xs font-medium text-[var(--qurai-muted)] sm:text-sm">{stat.label}</p>
                <p className="mt-0.5 text-xs text-[var(--qurai-quiet)]">{stat.count} ayat</p>
              </button>
            );
          })}
        </div>
      ...
      {activePopup && activeStat && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="qurai-card relative max-h-[80vh] w-full max-w-2xl overflow-hidden rounded-[1.5rem]">
              {/* Header */}
              <div className="border-b border-[var(--qurai-border)] bg-[color-mix(in_srgb,var(--qurai-green)_8%,transparent)] px-6 py-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={closePopup}
                    className="rounded-full p-2 text-[var(--qurai-muted)] transition hover:bg-[color-mix(in_srgb,var(--qurai-green)_10%,transparent)] hover:text-[var(--qurai-green)]"
                    aria-label="Tutup"
                  >
              ...
              <div ref={scrollContainerRef} className="max-h-[60vh] overflow-y-auto p-4">
                {popupVerses.length === 0 ? (
                  <p className="py-8 text-center text-[var(--qurai-muted)]">Tidak ada ayat dengan catatan ini.</p>
                ) : (
                  <div className="space-y-3">
                    {popupVerses.map((verse) => (
                      <button
                        ...
                        className="w-full rounded-xl border border-[var(--qurai-border)] bg-[color-mix(in_srgb,var(--qurai-surface-strong)_72%,transparent)] p-4 text-left transition hover:border-[var(--qurai-border-strong)] hover:bg-[color-mix(in_srgb,var(--qurai-green)_8%,transparent)]"
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <p className="line-clamp-3 flex-1 text-sm leading-6 text-[var(--qurai-muted)]">
